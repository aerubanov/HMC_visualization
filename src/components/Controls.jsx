...
function Controls({
  ...
  const [localAxisLimits, setLocalAxisLimits] = useState(axisLimits || { xMin: -5, xMax: 5, yMin: -5, yMax: 5 });
  const [useSeededMode, setUseSeededMode] = useState(false);

  useEffect(() => setDraftLogP(logP), [logP]);
  useEffect(() => setLocalBurnIn(burnIn), [burnIn]);
  useEffect(() => { if (axisLimits) setLocalAxisLimits(axisLimits); }, [axisLimits]);
  ...
  const handleAxisLimitChange = (key, value) => setLocalAxisLimits(prev => ({ ...prev, [key]: value }));
    ...
    <div className="controls">
      ...
      <div className="controls-content">
        ...
        <section className="control-section">
          <label htmlFor="logp-input" className="control-label">Probability Function</label>
          <div className="textarea-with-button">
            ...
            >
              ...
              {PREDEFINED_FUNCTIONS.map(f => (<option key={f.label} value={f.value}>{f.label}</option>))}
            </select>
            <button className="btn btn-apply" onClick={handleApplyLogP} disabled={!hasUnsavedChanges || !draftLogP.trim()}>
        ...
        <section className="control-section">
          ...
          <div className="control-group">
            <label className="control-label">Burn-in Samples</label>
            <input type="number" className="control-input" value={localBurnIn} onChange={e => {
          ...
          <div className="control-group">
            <div className="checkbox-group" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <input id="fast-mode-toggle" type="checkbox" checked={useFastMode} onChange={e => setUseFastMode(e.target.checked)} />
              <label htmlFor="fast-mode-toggle">Fast Sampling Mode</label>
            </div>
          </div>
          <div className="control-group">
            <div className="checkbox-group" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <input type="checkbox" id="second-chain-toggle" checked={chains.length > 1} onChange={e => {
          ...
          <div className="control-group">
            <div className="checkbox-group" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <input type="checkbox" checked={useSeededMode} onChange={e => {
                const en = e.target.checked;
                chains.forEach(c => setChainConfig(c.id, { seed: en ? (localSeeds[c.id] || 42) : null }));
              }} />
        ...
        {chains.map((chain, index) => (
          <section key={chain.id} className="control-section" style={{ borderLeft: `4px solid ${index===0?'#2c3e50':'#e74c3c'}`, paddingLeft: '8px' }}>
            ...
            <div className="control-row" style={{ display: 'flex', gap: '8px' }}>
              <div className="control-group" style={{ flex: 1 }}>
                <label className="control-label">Initial X</label>
                <input type="text" className="control-input" value={localPositions[chain.id]?.x ?? ''} onChange={e => {
                  setLocalPositions(p => ({...p, [chain.id]: { ...p[chain.id], x: e.target.value }}));
                  handlePosChange(chain.id, 'x', e.target.value);
                }} />
              </div>
              <div className="control-group" style={{ flex: 1 }}>
                <label className="control-label">Initial Y</label>
                <input type="text" className="control-input" value={localPositions[chain.id]?.y ?? ''} onChange={e => {
                  setLocalPositions(p => ({...p, [chain.id]: { ...p[chain.id], y: e.target.value }}));
            ...
            {useSeededMode && (
              <div className="control-group">
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <input type="number" className="control-input" style={{flex: 1}} value={localSeeds[chain.id] || ''} onChange={e => {
            ...
            <div style={{ marginTop: '1rem', display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
              {['xMin', 'xMax', 'yMin', 'yMax'].map(k => (
                <div key={k} className="control-group" style={{ width: '45%' }}>
                  ...
                  <input type="number" className="control-input" value={localAxisLimits[k]} onChange={e => handleAxisLimitChange(k, e.target.value)} onBlur={() => handleAxisLimitBlur(k)} />
                </div>
              ))}
            </div>
          </details>
        ...
        <section className="control-section">
          ...
          <button className="btn btn-primary" onClick={step} disabled={isRunning || !logP || useFastMode} style={{ width: '100%', marginBottom: '8px' }}>
            Step Once
          </button>
          <div className="sample-steps-group" style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
            <input type="number" className="control-input control-input-inline" style={{flex: 1}} min="1" value={nSteps} onChange={e => setNSteps(parseInt(e.target.value) || 1)} />
            <button className="btn btn-accent" style={{flex: 2}} onClick={() => sampleSteps(nSteps)} disabled={isRunning || !logP}>
              Sample N Steps
            </button>
          </div>
          <button className="btn btn-secondary" onClick={reset} disabled={isRunning} style={{ width: '100%' }}>
            Reset Sampler
          </button>
        </section>
